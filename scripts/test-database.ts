...
async function runDatabaseTest() {
  ...
  try {
    ...
    try {
      const applications = await executeQuery("SELECT COUNT(*) as count FROM applications")
      const count = (applications as any)[0].count
      console.log(`📊 Applications table: ${count} records`)

      // Test sample query
      const sampleApps = await executeQuery("SELECT id, nome_completo, email, status FROM applications LIMIT 3")
